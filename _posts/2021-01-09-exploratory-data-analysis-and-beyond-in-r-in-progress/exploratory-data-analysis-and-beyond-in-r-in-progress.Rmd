...
The symmetry can be described by two measures: **skewness and kurtosis**. They are useful, because **significant skewness and kurtosis** clearly indicate that data are not normal.
...
There are two main ways to check the normality: using a **Quantile-Quantile plot or a statistical test**.
...
Cool, right? But **plot_normality()** function from {dlookr} package **visualizes** not only **Histogram** and **Quantile-Quantile of the original data**, but also **two of the most common transformations of data**, namely **log transformation & square root**, in case the normality assumption wasn't met. This allows us to see, whether transformation actually improves something or not. Here we could also use {dplyr} syntaxis in order to quickly visualize several groups.
...
```{r}
normality(airquality) %>% flextable()
```


Moreover, via the collaboration with {dplyr} package and it's *group_by()* function we can conduct around 2000 normality tests in seconds and only few line of code:
...
For example, using the intuitive *plot_boxplot()* function from {DataExplorer} package with an argument **by = ** which specifies a categorical, or grouping, variable, will put **all groups of all numeric variables into the boxes**. Such exploration immediately creates the next question - **are these groups differ significantly?** We can't tell from just staring at the picture...
...
...but if use a *ggbetweenstats()* function from {ggstatsplot} package, we'd check tons of hypothesis using only few intuitive arguments:
...
This simple code not only provides you with a p-value which tells you whether there are significant differences between groups, but also conducts multiple pairwise comparisons to see between which groups exactly these differences are, and even adjusts the p-values for multiple comparisons with Holm method automatically. Moreover, it also produces bunch of other statistical details on top of amazing visualization. If fact, I found *ggbetweenstats()* function sooo useful, that I did two separate videos on it already.
...
{dlookr's} **plot_correlate()** function is even more useful, because it **visualizes these relationships**. We can of course determine the **method of calculations**, be it a default "pearson", or a non-parametric "kendall" or "spearman" correlations, which are more appropriate for **non-normally and non-very-linearly distributed values with some outliers**. The **shape** of each subplot shows the **strength of the correlation**, while the **color** shows the **direction**, where blue is **positive** and red is **negative correlation**. It's fine for the pure exploratory analysis, but, as always, the next logical step would be to test hypothesis and figure out which correlations are significant.
...
- a colored heatmap showing positive or negative correlations and finally
- whether a particular correlation is significant or not, where not-significant correlations are simply crossed out.

Moreover, we can get the results in a table form with p-values and confidence intervals for correlation coefficients, if we need to.
...
However, my favorite way to conduct correlation analysis is supported by the *chart.Correlation()* function from {PerformanceAnalytics} package. It displays not only (1) correlation coefficients, but also (2) **histograms** for every particular numeric variable and even (3) **scatterplots** for every combination of numeric variables, which show whether relationships are non-linear and whether correlation analysis makes sense at all. Besides, the (4) significance stars are particularly helpful, because they describe the strength of correlation. Here we can of coarse also specify the (5) method we measure the correlation by.
...
If the distribution and the spread of data does not of a big interest for you, you can use *cor_sig_star()* fucntion from {fastStat} package, which displays all the useful information in a form of a table with correlation coefficients and p-values.
...
The *compare_numeric()* function examines the relationship between numerical variables with the help of **(Pearson's) correlation** and **simple linear models**. The correlation results are a little boring because they only provide correlation coefficients (therefore not shown). However, the results of pairwise linear regressions are interesting, because they not only produce p-values, but also other useful metrics, like $R^2$, AIC etc. On top of this, we could plot all the results of *compare_numeric()* function showing: 

- the strength of the correlation with circles,
- the spread of data with box-plots and 
- the linear regression, just to quickly see whether it makes sense
...
However, how can we explore the whether linear model makes sense at all? Wenn the easiest way is to plot the data with {ggplot2} package and use *geom_smooth()* function to display a non-linear model fit of usually generalized additive models:
...
I found this topic actually so cool, that I produced a separate video and [article](https://yuzar-blog.netlify.app/posts/2021-03-04-how-to-impute-missing-values-in-r/) on it while doing this one. So, for a deeper insight on how to impute numeric and categorical missing values with fancy machine learning methods, check out the other video on my channel. 

Here I just give you two useful functions from {dlookr} package. The first one - *plot_na_intersect()* shows you which variables have missing values and how many. And the second function *imputate_na()* imputes missing values with several machine learning methods.
For instance, using "K nearest neighbors" algorithm, we could impute 37 missing values in Ozone variable, and even visually check the quality of our imputation in only one line of code. Using the *imputate_na()* function, we only need to specify 4 arguments:
...
*check_outliers()* fucntion from {performance} package provides an easy way to identify and visualize outliers with different methods. If you want to have an aggressive method and cleap out a lot of outliers, go with the *zscore* method, but if you don't have much data, go with less conservative method, for example *interquartile range*.
...
Similarly, using *imputate_outlier()* fucntionn from {dlookr} package, we can impute outliers with several methods:
