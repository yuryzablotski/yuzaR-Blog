...
description: |
  Chi-Square test checks the independence between two categorical variables. Both variables can have two or more categories.
author:
  - name: Yury Zablotski
...
## This post as a video (in progress)
...
If you have installed and loaded `ggstatsplot` package, you can use `ggbarstats` function to conduct and visualize Chi-Square test of independence between two categorical variables. Withing this function we need to specify only four arguments: 

- **our data**, for example let's take `mtcars`, which you already have in R, so you don't need to look for it
...
- **Chi-Square statistics** was previously used to calculate p-value, but nowadays, since p-values are always calculated by computers, we can safely ignore it
...
- That's why we have **V Cramer** value with its 95% confidence intervals as **the effect size** next to p-value. Our effect size of 0.46 indicates a **relatively strong relationship**, which supports the conclusion made by the p-value. The confidence intervals do not make much sense though, since *V cramer* goes from 0 to 1.

![](v_cramer.png)

- However, `ggbarstats` provides a second **Bayesian V Cramer effect size**, which delivers much more useful 95% Highest Density Intervals. The interpretation of the Bayesian effect size is the same, so the relationship between our variables is **relatively strong**.
...
- We can also see the **Proportion Test** for transmissions to be carried out fo each cylinders category. It shows whether proportions inside every cylinder differ. Our Null Hypothesis ($H_0$) here is that there are equal proportions of different transmissions in a particular category of cylinder. Our Alternative Hypothesis ($H_alt$) is that the proportions differ.

## Pairwise Chi-Sqared Tests ... or post-hoc tests

If you find a significant relationship between variables and you have more then two categories in any of your variables, like in our example, we might be interested to compare proportions of cylinders with each other, namely 4 with 6, 4 with 8 and 6 with 8. Such simple pairwise comparisons is often called with an unnecessary fancy name - **post-hoc tests**.
