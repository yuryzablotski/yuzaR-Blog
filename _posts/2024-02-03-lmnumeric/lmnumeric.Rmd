...
It's weak because age is not the most important predictor for salary ;). Education is, but we'll see more in the next video on this series, where I show how to model a categorical predictor with linear model. Here, we can quickly visualize linear models for every education group with the help of the "geom_smooth()" function. Namely, the folks with "Advanced Degree" start with the salary of ca 120K while people without High-School start at 70K. 

```{r}
ggplot(d, aes(x = age, y = wage, color = education))+
