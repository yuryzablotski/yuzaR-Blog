...
{DataExplorer} package provides very intuitive functions for getting histogram and density plots of all continuous variables at once, namely *plot_histogram()* and *plot_density()*. {SmartEDAs} function *ExpNumViz()* provides nice looking density plots with **Skewness** and **Kurtosis**.
...
However, {ggpubr} package shows the confidence intervals, which help to decide whether the deviation from normality is huge or not. However, in order to proverly check the normality, we'd need to actually do a statistical test for normality
...
So, *normality()* function from {dlookr} package performs Shapiro-Wilk normality tests with every numeric variable in the dataset. Moreover, via the collaboration with {dplyr} and therefore *group_by()* function we cat conduct ca. 2000 normality tests in seconds:

```{r}
normality(iris) %>% knitr::kable()
```
...
- Quantile-Quantile plot of the original data
