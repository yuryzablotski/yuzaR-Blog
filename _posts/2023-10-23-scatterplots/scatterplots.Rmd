...
# This post as 14 minutes video (in progress)
...
After loading packages and data and renaming some variables for our convenience, we can create the scatter plot by employing the ‘ggplot()’ function. This function requires two arguments: the ‘data’ and the ‘aesthetic’ mappings. These aesthetic mappings determine the variables to be mapped onto the x- and y-axes of the plot. Lastly, we’ll utilize the ‘geom_point()’ function to display points that connect wealth on the x- with happiness on the y-axes.
...
Outliers are data points that significantly differ from the rest of the data. These outliers may represent errors or anomalies in the data, unusual events, or even interesting findings. In any case, they can skew our results, so it's important to clearly identify them. And there's a simple way to achieve that - by making this plot interactive. For that, we’ll: 
...
# 6. Multivariate Exploratory Analysis (MEA Ziegengeschrei ;) to tell a compalling story

Now, since "color" provided completely new information to our two-dimensional plot, we could see "color" argument as a third dimension. Similarly, we can introduce five additional dimensions, which will transform a simple scatter plot into a captivating narrative. Think of it as a form of Multivariate Exploratory Analysis (MEA Ziegengeschrei ;), but without the complexity of hardcore statistics. And here is how.
...
```{r eval = F}
set.seed(2)
ggplot(mea %>% filter(country %in% c("United States", "Venezuela") & year %in% c(2015:2023)), aes(x = life_expectancy, y = happiness, 
                                                                                                  
ggplot(d %>% 
         filter(country %in% c("United States", "Venezuela") & year %in% c(2015:2023)), aes(x = life_expectancy, y = happiness, color = country)) +
  geom_point()+
  geom_smooth(method = lm)+ 
  stat_ellipse()+
  geom_label(aes(label = year), size = 1)
```


This adds an even more intriguing layer to our story. Specifically, only the Germans appear in every subplot. What's the common thread? Well, it turns out that most Germans have someone they can rely on in times of trouble. However, when it comes to generosity, they're split right down the middle – with half of them displaying a generous spirit and the other half leaning toward thriftiness. Having spent half of my life in Germany, I can certainly relate to this.
...
Finally, once you're satisfied with your visualization, you can save it using the "ggsave" command in your preferred format, quality, and size. And if you are satisfied with this video so far, please consider hitting a like button, it helps the channel a lot. 
...
<!-- Excelennt for quick data exploration. -->

```{r eval=FALSE, fig.height=11, fig.width=11, include=FALSE}
...
```{r fig.width=11, fig.height=11, eval = F, eval = F, include=F}
# install.packages("GGally")
library(GGally)
ggpairs(d,
        columns = 2:11,
