---
title: "R package reviews {emmeans} Interactions - unleash the power of {emmeans}"
description: |
...
For instance, if someone asks you, "Do you prefer to wear a raincoat or a sweater on a cold day?" Your response would probably be, "It depends on whether it's raining or not!" In this case, the *precipitation* is the other variable in the interaction term, and it affects your preference for wearing a *raincoat or a sweater*. This shows the "it depends" nature of an interaction effect, where you can't answer the question without considering the other variable in the interaction term.

# Interaction between two categorical predictors

Let's compare a model with to a model without interaction to see how. These models explore, whether we'll earn more money as we age and see how a profession choice affects our salary. 
...
However, the model has a problem: salary increases with age regardless of profession, resulting in identical differences between young and old in both job classes. Similarly, IT professionals earn 21.3K more than factory workers at any age, young or old. This seems unrealistic!
...
> **_NOTE:_** Buy the way, if you stumbled upon this video and some words or code are unclear, I highly recommend watching the [introductory video on {emmeans} first](https://youtu.be/hyP3Hx_1kTM).
...
As a result, young people earn roughly the same amount regardless of their health, while older, healthier individuals will earn approximately 53K per year more at the end of their career, around age 80, which IS significant.
...
Check this out! If we use the "cov.reduce = FALSE" argument with "emmip" function, we will be able to see all possible values of age. This gives us a helpful overview of our data and enables us to choose fewer more meaningful values to compare. The "at = list()" argument allows us to specify exact ages. Let's take 25, 45, and 65 years, representing the beginning, middle, and end of a career, as an example.  
