...
bla <- ggplot(Wage, aes(x = wage)) +
  ...
  geom_vline(data = wage_stats,         
             ...
             linetype="dotted", color = "red")+
  theme_void()+ 
  geom_label(aes(x = 110, y = 450, size = 3, fontface = "bold", 
                label = paste("Mean:", round(wage_stats$mean_wage) )))
...
I hope you now see that displaying data in the form of histograms, density curves, and central tendencies is a useful tool for making decisions. For example, we could test our mean (in blue) against the mean of other people (in red) using a one-sample t-test to determine if our result significantly differs. However, one-sample t-tests open up a completely new chapter, and you can explore all the intricacies right here in this video.
