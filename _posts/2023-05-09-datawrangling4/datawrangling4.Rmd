...
```{r}
x <- tibble(A = c("a", "b", "c"), B = c("one", "two", "three"))

y <- tibble(A = c("a", "b", "d"), B = c("one", "two", "four"),  C = c(3, 2, 1) )

z <- tibble(A = c("a", "b", "d"), B = c("one", "two", "four"),  D = c(3, 2, 1) )

x2 <- tibble(A = c("a", "c"), B = c("one", "three"))
```


```{r message=FALSE, warning=FALSE, echo=F}
library(gridExtra)
gridExtra::grid.arrange(tableGrob(x), tableGrob(y), tableGrob(z), tableGrob(x2), ncol = 4)
```


# Bind rows and bind columns

Sometimes we just need to combine several tables into one, by either combining their columns or their rows. "bind_cols()" is the most intuitive way to combine columns.

```{r message=FALSE, warning=FALSE, echo=F}
gridExtra::grid.arrange(tableGrob(x), tableGrob(y), ncol = 2)
```
...
```{r message=FALSE, warning=FALSE, echo=F}
gridExtra::grid.arrange(tableGrob(x), tableGrob(x2), ncol = 2)
```
...
If you want to put two tables below each other, **bind_rows()** is here to help. The tables **need to have the same column names** thought, otherwise **bind_rows()** will produce *NAs* in your new dataset, where column names don't match. If you have large tables and more than two of them, you might completely lose track of which data belongs to which table. To prevent this, the **.id** argument allows you to track which table particular values belong to.
