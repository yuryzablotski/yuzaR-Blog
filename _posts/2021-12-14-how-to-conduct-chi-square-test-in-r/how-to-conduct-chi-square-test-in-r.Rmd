...
description: |
  Chi-Square test checks the independence between two categorical variables, where variables can have two or more categories. Take a close look at this statistics rich picture. It looks like a lot of work, but it can actually be done with only one line of code. So, in this video we'll learn how to do that and how to interpret all these results. Let's get into it.
author:
  - name: Yury Zablotski
...
- first, **our data**, for example let's take `mtcars`, which you already have in R, so you don't need to look for it
...
- We can also see **proportion Tests** for transmissions in each cylinder. They show whether proportions inside every cylinder differ. Our Null Hypothesis ($H_0$) here is that there are equal proportions of different transmissions in a particular category of cylinder. Which is the case for cylinders 4 and 6. While, our Alternative Hypothesis ($H_alt$) is that the proportions differ, which is the case for cylinder 8.
...
So, thanks for watching. If you did like this video, feel free to
subscribe and I'll catch you again soon for the next one.
