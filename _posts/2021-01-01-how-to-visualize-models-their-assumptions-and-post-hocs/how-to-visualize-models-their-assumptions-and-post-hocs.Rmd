...
Let's get our first data. We'll use several dataset though.
...
The *effects()* package is the mane visualization package we'll use. It can visualize results of almost any model. We'll start with the visualization of predicted values.
...
Here we'll add a *grid = TRUE* for a better readability, so that we don't need to stare on the y-axis and guess the result.
...
We can visualize any particular predictor from a multiple model individually.
...
We can change design of the plot by determining the number of *rows* and *columns*. The *sjPlot* package can also display the estimates themselves, as numeric values, and the significance start, which is often all we need. Besides, it looks much better than a table-looking output of the model results.
...
We do not always need to explicitly model things in order to explore our data. A *geom_smooth()* from *ggplot2* package will automatically fit numeric data. Mostly with non-linear models (e.g. GAM), but with the argument *method="lm"* forces it to take the linear one. Moreover, we could also use any formula inside of the *geom_smooth()*, but since we can't really model categorical predictors, and when we need to write formula anyway, we then better produce an actual model. However, the code presented below is quick and easy, and therefore very practical! On this note we'll enter the world of non-linear models and I recommend to watch the second video first, before checking out the code.
...
The *allEffects* function can also easily handle non-linear models. For instance polynomial ones. In contrast, the *plot_model* function can't. However, we
...
- *mgcv* is another package for GAMs but there are still conflicts! Thus, use *gam::* to specify from which package exactly the *gam()* function should be used. The *gam* package has it's own plotting function - *plot.Gam* and you can display all subplots on one plot by using *par(mfrow = c(2, 2))*. Please, don't use "plot(allEffects(gam1))" for GAM models, because it will produce linear results.
...
And here is promise visualization of the post-hocs. Notice the marginal means on the y axis and color-coded design of the plot, which connects the pairs the p-value value for which was calculated (x-axis).
...
The mixed-effects models are very complex. But fortunately, both *plot(allEffects())*, *plot_model()*, *emmeans()* and *check_model()* work flawlessly and **simply deliver!** The *check_model()* even check the assumptions for the random effects! The random effects themselves can also be visualized, but they are rarely interpreted, so, why bother?
...
These kind of models are just madness, can be visualized though.
...
Survival models are can be visualized with *survminer* package. Even some statistical details can be displayed. I still did not figured out how to visualize the post-hocs for survival analysis. It you know how, please let me know via YouTube. Thus, I here simply provide the p-values of the post-hocs.
...
The are more common as compared to the exponential models and can be visualized with beautiful *ggforest* plots.
